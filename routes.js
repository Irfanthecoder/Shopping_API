...
router.put("/product/update/:productId", async (req, res) => {
    try {
      ...
      const [result] = await conn.promise().execute(queryString, [name, categoryId, price, description, productId]);
  ...
  router.delete("/product/delete/:productId", async (req, res) => {
    try {
      ...
      const [result] = await conn.promise().execute(queryString, [productId]);
